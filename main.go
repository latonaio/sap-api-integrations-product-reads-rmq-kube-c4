...
import (
	sap_api_caller "sap-api-integrations-product-reads-rmq-kube-c4/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-product-reads-rmq-kube-c4/SAP_API_Input_Reader"
	"sap-api-integrations-product-reads-rmq-kube-c4/config"
...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	productID := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetProduct(productID, accepter)
	return nil
}

func extractData(data map[string]interface{}) (productID string) {
	sdc := sap_api_input_reader.ConvertToSDC(data)
	productID = sdc.ProductCollection.ProductID
	return
}
